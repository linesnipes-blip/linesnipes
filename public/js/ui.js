    ...
    open ? h('div', { style: { padding: '0 14px 14px', animation: 'fadeIn .2s' } },
      ...lines.map(l => {
        ...
        return h('div', { style: {
          ...
          ...(isH ? { paddingTop: '6px', borderTop: '1px solid rgba(191,189,242,.1)' } : {}),
        } }, l);
      })
    ) : null,
  );
...
function parlayResults() {
  ...
  return h('div', { cls: 'card', style: { borderColor: 'rgba(191,189,242,.12)', background: 'linear-gradient(180deg,rgba(191,189,242,.025) 0%,transparent 100%)', animation: 'fadeUp .4s ease' } },
    ...
    : h('div', { style: { display: 'grid', gap: '8px' } },
      ...results.map((p, idx) => {
        ...
        return h('div', { style: {
          background: idx === 0 && pos ? 'rgba(191,189,242,.05)' : 'var(--card)',
          border: idx === 0 && pos ? '1px solid rgba(191,189,242,.12)' : '1px solid var(--border)',
            ...
            h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
              h('div', {},
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '4px' } },
                  ...p.legs.map((l, i) => h('span', { style: { fontSize: '10px', background: 'rgba(191,189,242,.08)', color: 'var(--accent)', padding: '2px 8px', borderRadius: '4px', fontFamily: 'var(--mono)' } },
              ...
              h('div', { style: { textAlign: 'right', marginLeft: '12px' } },
                ...
                h('div', { style: { fontSize: '10px', color: pos ? '#BFBDF2' : '#ff6b6b', fontWeight: '600', marginTop: '2px', fontFamily: 'var(--mono)' } },
                  (pos ? '+' : '') + '$' + p.ev.toFixed(2))),
            )),
          open ? h('div', { style: { padding: '0 14px 14px', animation: 'fadeIn .2s' } },
            h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px', marginBottom: '8px' } },
              ...
              h('div', { cls: 'sbox' }, h('div', { cls: 'sl' }, 'Edge'), h('div', { cls: 'sv', style: { color: pos ? '#BFBDF2' : '#ff6b6b' } }, (pos ? '+' : '') + p.evPct.toFixed(2) + '%'))),
      ...
      ...games.map(g => {
        ...
        return h('div', { cls: 'gcard' + (active ? ' on' : ''), style: !hasBook ? { opacity: '.4' } : {} },
          ...
          active && gr ? h('div', { style: { padding: '0 12px 12px', animation: 'fadeIn .25s' } },
            h('div', { style: { borderTop: '1px solid rgba(191,189,242,.1)', paddingTop: '10px', marginTop: '2px' } },
              ...
              : h('div', { style: { display: 'grid', gap: '5px' } },
                ...gr.map((r, idx) => {
                  ...
                  return h('div', { style: {
                    background: idx === 0 && pos ? 'rgba(191,189,242,.05)' : 'rgba(255,255,255,.02)',
                    border: idx === 0 && pos ? '1px solid rgba(191,189,242,.12)' : '1px solid rgba(255,255,255,.04)',
                      ...
                      h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
                        h('div', { style: { flex: '1' } },
                          h('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '2px' } },
                            idx === 0 && pos ? h('span', { style: { fontSize: '8px', fontWeight: '700', background: 'rgba(191,189,242,.15)', color: 'var(--accent)', padding: '1px 6px', borderRadius: '3px', letterSpacing: '.5px', textTransform: 'uppercase' } }, 'Best') : null,
                    ...
                    open ? h('div', { style: { padding: '0 12px 12px', animation: 'fadeIn .2s' } },
                      h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px', marginBottom: '8px' } },
                        ...
                        h('div', { cls: 'sbox' }, h('div', { cls: 'sl' }, 'EV'), h('div', { cls: 'sv', style: { color: pos ? '#BFBDF2' : '#ff6b6b' } }, (pos ? '+' : '') + '$' + r.ev.toFixed(2)))),
