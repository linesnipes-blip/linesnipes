        ...
        return h('div', { style: {
          ...
          h('button', { style: { width: '100%', textAlign: 'left', padding: '12px 14px', fontFamily: 'inherit', cursor: 'pointer', background: 'none', border: 'none', color: 'var(--fg)' },
            ...
            h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
              h('div', {},
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '5px', marginBottom: '6px' } },
                  ...p.legs.map((l, i) => {
                    const legEV = ((l.bookDecimal / l.fairDecimal) - 1) * 100;
                    const legPos = legEV > 0;
                    return h('div', { style: { display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '11px', background: 'rgba(155,201,242,.06)', border: '1px solid rgba(155,201,242,.1)', padding: '5px 10px', borderRadius: '6px' } },
                      h('span', { style: { color: '#fff', fontWeight: '600', fontFamily: 'var(--display)' } },
                        l.outcome + (l.point != null ? ' (' + (l.point > 0 ? '+' : '') + l.point + ')' : '')),
                      h('span', { style: { color: 'var(--fg3)', fontSize: '9px' } }, '·'),
                      h('span', { style: { color: 'var(--fg2)', fontFamily: 'var(--mono)', fontSize: '10px' } },
                        l.marketLabel + ' ' + amOdds(l.bookDecimal)),
                      h('span', { style: { color: legPos ? '#00c853' : '#ff4757', fontWeight: '700', fontFamily: 'var(--mono)', fontSize: '10px' } },
                        (legPos ? '+' : '') + legEV.toFixed(1) + '%'),
                    );
                  })),
                h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: '8px', fontSize: '10px', color: 'var(--fg3)' } },
                  h('span', {}, p.uniqueGames + ' game' + (p.uniqueGames > 1 ? 's' : '')),
                  h('span', {}, 'Combined: ' + amOdds(p.parlayDecimal)),
                  p.avgLegEV != null ? h('span', {}, 'Avg Leg EV: ' + (p.avgLegEV > 0 ? '+' : '') + p.avgLegEV.toFixed(1) + '%') : null)),
